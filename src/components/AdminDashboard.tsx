...
export const AdminDashboard = () => {
  const { profile, loading: authLoading, user } = useSupabase();
  ...
  useEffect(() => {
    ...
    const fetchInitial = async () => {
      const [propsRes, inqRes, usersRes, verifRes, authLogsRes] = await Promise.all([
        supabase.from('properties').select('*').order('created_at', { ascending: false }),
        supabase.from('inquiries').select('*').order('created_at', { ascending: false }),
        supabase.from('profiles').select('*').order('created_at', { ascending: false }),
        supabase.from('agent_verification_requests').select('*').order('created_at', { ascending: false }),
        supabase.from('audit_log_entries').select('*').order('created_at', { ascending: false })
      ]);
      if (propsRes.data) setProperties(propsRes.data);
      if (inqRes.data) setInquiries(inqRes.data);
      if (usersRes.data) setUsers(usersRes.data);
      if (verifRes.data) setVerificationQueue(verifRes.data);
      if (authLogsRes.data) setAuditLogs(authLogsRes.data);
      setLoading(false);
    };
    fetchInitial();

    // Module 1: Live Properties
    const propertiesChannel = supabase.channel('admin-properties-queue')
    ...
    // Module 2: Live Inquiries
    const inquiriesChannel = supabase.channel('admin-inquiries-feed')
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'inquiries' }, (payload) => {
        setInquiries(prev => [payload.new, ...prev]);
      }).subscribe();

    // Module 3: Verification Queue
    const verificationChannel = supabase.channel('verification-queue')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'agent_verification_requests' }, (payload) => {
        setVerificationQueue(prev => {
          if (payload.eventType === 'INSERT') return [payload.new, ...prev];
          if (payload.eventType === 'UPDATE') return prev.map(q => q.id === payload.new.id ? payload.new : q);
          return prev;
        });
      }).subscribe();

    // Module 4: Audit Logs
    const auditStream = supabase.channel('audit-log-stream')
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'audit_log_entries' }, (payload) => {
    ...
    return () => {
      ...
      supabase.removeChannel(auditStream);
    };
  }, [authLoading, profile]);
      ...
      <aside className="w-72 bg-primary text-white p-8 space-y-12 shrink-0 border-r border-white/5">
        ...
        <nav className="space-y-2">
          {[
            ...
            { id: "verification", icon: <Check className="w-4 h-4" />, label: "Verifications" },
            { id: "users", icon: <Users className="w-4 h-4" />, label: "Users" },
            { id: "audit", icon: <LayoutDashboard className="w-4 h-4" />, label: "Audit Logs" },
            ...
            <button
              ...
              className={`w-full flex items-center gap-3 px-4 py-3 rounded-custom text-sm font-bold uppercase tracking-widest transition-all ${activeTab === item.id ? "bg-secondary text-primary" : "text-white/60 hover:bg-white/5"
                }`}
            >
                    ...
                    {inquiries.map((inq) => (
                      <div key={inq.id} className="bg-white p-8 rounded-custom border border-gray/10 shadow-sm flex flex-col md:flex-row justify-between gap-8">
                        <div className="space-y-4">
                          <div className="flex items-center gap-3">
                            <span className={`px-3 py-1 rounded-full text-[10px] uppercase font-bold tracking-widest ${inq.status === 'pending' ? 'bg-yellow-100 text-yellow-700' :
                              inq.status === 'contacted' ? 'bg-blue-100 text-blue-700' : 'bg-green-100 text-green-700'
                              }`}>
                              {inq.status}
                            </span>
                            <span className="text-xs text-gray/40 uppercase tracking-widest">{new Date(inq.created_at).toLocaleDateString()}</span>
                          </div>
                      ...
                      <tr key={u.id} className="hover:bg-gray-50 transition-colors">
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-4">
                            ...
                            <span className="font-bold text-primary">{u.full_name || 'Anonymous'}</span>
                          </div>
                        </td>
                        ...
                        <td className="px-6 py-4">
                          <span className={`px-3 py-1 rounded-full text-[10px] uppercase font-bold tracking-widest ${u.role === 'admin' ? 'bg-secondary text-primary' : 'bg-gray-100 text-gray-600'
                            }`}>
                            {u.role}
                          </span>
                        </td>
